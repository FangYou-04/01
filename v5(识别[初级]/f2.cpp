    ...
    {           
        for (size_t j = i + 1 ; j < Edge.size(); j++)
        {
            ...
            armor.boundingRect = cv::Rect(
                ...
                std::max(rect1.y + rect1.height, rect2.y +
...
void drawcontours(cv::Mat &img, std::vector<light> Edge, std::vector<Armor> armors)
{
    for(const auto &l : Edge)
    {
        ...
        for (int k = 0; k < 4; k++)
        {
            cv::line(img, pts[k], pts[(k+1)%4], cv::Scalar(0,255,0), 3);
        }
    ...
    {
        cv::rectangle(img, a.boundingRect, cv::Scalar(0,255,0), 3);
        cv::circle(img, a.center, 5, cv::Scalar(255,0, 0), -1);
    }
...
int main()
{
    cv::VideoCapture cap("src/5.mp4");
    if (!cap.isOpened())
    {
        std::cout << "视频打开失败！"; return -1;
    }
